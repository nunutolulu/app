...
export default {
  ...
  computed: {
    ...
    // 给子组件传数据——服务器的数据没有回来，则返回的是空对象，至少保证其不为undefined
    skuImageList() {
      return this.skuInfo.skuImageList || []
    }
  },
  mounted() {
    this.$store.dispatch('detail/getDeatilInfo', this.$route.params.skuid)
  },
  methods: {
    ...
    changeSkuNum(event) {
      ...
      // 用户输入进来的文本*1，如果是字符串等形式相乘之后返回的是NaN
      let value = event.target.value * 1;
      // 如果输入进来的非法，出现Nan或者小于1
      if (isNaN(value) || value <= 0) {
        this.skuNum = 1;
      } else {
        // 正常大于1，但是排除小数的情况，使用parseInt
    ...
    async addShopCart() {
      ...
      /* let result = await this.$store.dispatch('detail/addOrUpdateShopCart', { skuId: this.$route.params.skuid, skuNum: this.skuNum })
      console.log(result); */
      // 2、服务器存储成功——进行路由跳转
      try {
        await this.$store.dispatch('detail/addOrUpdateShopCart', { skuId: this.$route.params.skuid, skuNum: this.skuNum })
        // 路由跳转
        // 一些简单的额数据【比如skuNum】，通过query给路由组件传递过去
        // 产品的信息【比较复杂：skuinfo】，通过会话存储（不持久化，会话结束数据消失）
        // 本地存储|会话存储，一般存储的是字符串
